...
async def test_full_story(world):
    ...
    async with world.pool() as s:
        ...
        await s.commit()
        delivered_tg = await svc.notify_responsibles(world.bot, _core, sub, "ru")
...
Check the real name of the Telegram fan-out method in `bot/services/submissions.py` (`notify_responsibles` at ~line 128 — confirm the signature `(bot, core, sub, default_locale)`) and the `authorize(..., require_owner=True)` semantics for admins in `bot/services/actions.py`; adjust the call accordingly.
